<?php
	...
	function getJSData()
	{
		...
		if( $C->THEME_NAME != 'lovely_ume' ){
			if ($handle = @opendir($C->INCPATH.'../themes/'.$C->THEME_NAME.'/js/')) {
	
				while (FALSE !== ($entry = readdir($handle))) {
					if( $entry !== '.' && $entry !== '..' ){
						$html .= '<script type="text/javascript" src="'.$C->SITE_URL.'themes/'.$C->THEME_NAME.'/'. $entry .'?v='.$C->VERSION.'"></script>'."\n";
					}
				}
	...
	function getCSSData()
	{
		...
		if( $C->THEME_NAME != 'lovely_ume' ){
			if ($handle = @opendir($C->INCPATH.'../themes/'.$C->THEME_NAME.'/css/')) {
	
				while (FALSE !== ($entry = readdir($handle))) {
					if( $entry !== '.' && $entry !== '..' ){
						$html .= '<link href="'. $C->SITE_URL .'themes/'. $C->THEME_NAME .'/'. $entry .'.css?v='. $C->VERSION .'" type="text/css" rel="stylesheet" />'."\n";
					}
				}
	...
	function createTagLinks($tag_array)
	{
		...
		foreach( $tag_array as $t ){
			$html .= '<a href="'. $C->SITE_URL .'search/tab:tags/s:'. $t .'" title="#'. htmlspecialchars($t) .'"><small>#</small>'. htmlspecialchars(str_cut($t,25)) .'</a> ';
		}
		$html .= '</div>';
